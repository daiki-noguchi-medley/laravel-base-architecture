...
│   │   ├── User/
│   │   │   ├── UserAuth.php               Authenticatable 実装 (User Model を包む)
...
│   ├── Http/
│   │   ├── Controller/                    薄い (Request → Service → Resource/ViewModel)
│   │   │   ├── User/
...
│   │   │       ├── Auth/LoginController.php
│   │   │       └── DashboardController.php
│   │   ├── Request/                       FormRequest (フィールド public const string)
...
│   ├── Model/                             テーブル 1 枚 = 1 クラスの readonly Model
│   │   ├── User/User.php                  (Eloquent ではない素の PHP オブジェクト。
│   │   ├── Admin/Admin.php                 ドメインサブフォルダ必須、直置き禁止)
│   │   └── Kanban/KanbanCard.php
│   ├── Enum/
│   │   └── Kanban/KanbanLane.php
│   └── Providers/
│       ├── AppServiceProvider.php         boot 専用 (Auth::provider() 登録。bind は書かない)
│       ├── RepositoryServiceProvider.php  Repository interface→Impl の DI バインド
│       └── ServiceServiceProvider.php     Service interface→Impl の DI バインド
├── Demo/                                  事業ドメインパッケージ (CLAUDE.md §4)
│   ├── User/                              ← ドメイン先頭、その下にレイヤー
│   │   ├── Service/
...
│   │                                      (stdClass → Model 変換は private toModel())
│   └── Admin/
│       ├── Service/
...
| Auth 層 | `App\Auth\User\UserAuth` | `Authenticatable` interface 実装、`User` Model を包む readonly オブジェクト |
| Service 層 | `Demo\User\Service\UserAuthService` | 新規登録 (`Hash::make` してから Repository に渡す) |
| Repository 層 | `Demo\User\Repository\UserRepository` | interface (`findById`, `findByEmail`, `insert`, `updateRememberToken`) |
| Repository 層 | `Demo\User\Repository\UserRepositoryImpl` | **`DB::table('user')` クエリビルダー実装**。stdClass → Model 変換は `private function toModel(stdClass $row)` が担う |
| Model | `App\Model\User\User` | テーブル 1 行を表す readonly Model (Eloquent モデルを Service に漏らさないための変換層) |

Admin も同じ構造 — `App\Auth\Admin\` / `Demo\Admin\Service\` / `Demo\Admin\Repository\`。

### ガード設定 (`config/auth.php`)

```php
'guards' => [
    'web'   => ['driver' => 'session', 'provider' => 'user'],   // デフォルトガードも自前 provider を使う
...
[Browser]
   ↓ HTTP リクエスト
[Controller (app/Http/Controller/)]                     薄く
   ├─ FormRequest (app/Http/Request/) で自動検証
   ↓ Service にプリミティブ / DTO で渡す
[Service (Demo/<ドメイン>/Service/)]                    ビジネスロジック
   ├─ トランザクション境界はここ
   ↓ Repository 経由で外部と通信
[Repository (Demo/<ドメイン>/Repository/)]              永続化抽象
   ├─ DB: DB::table('user')->... (クエリビルダー基本)
   └─ 外部 API: Http::post(...)
   ↓ 結果は Model (app/Model/<ドメイン>/、readonly) で返す
...
> Eloquent ひな型の `app/Models/User.php` と `database/factories/UserFactory.php` は **削除済み**。
> `config/auth.php` の `web` ガードを自前 provider `'user'` (driver `userauth`) に切り替えているため、
> 削除しても Laravel の暗黙参照は壊れない。
